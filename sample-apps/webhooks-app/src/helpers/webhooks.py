...
from hubspot.webhooks import (
    SubscriptionCreateRequest, BatchInputSubscriptionBatchUpdateRequest,
    SubscriptionPatchRequest, SettingsChangeRequest, ApiException,
...
def pause_active_subscriptions(hubspot_client: Client, app_id: str):
    ...
    inputs = [SubscriptionBatchUpdateRequest(id=s.id, active=False) for s in active_subscriptions]
...
def create_or_activate_subscription(hubspot_client: Client, app_id: str, event_type: str, property_name=None):
    try:
        ...
        existing_subscription_id = json.loads(json.loads(e.body)['context']['subscriptionIds'][0])[0]
